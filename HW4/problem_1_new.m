...
tic
for i = 0:(floor(length(llr)/sstep)) - 1
    %decodes block by block the input bits
    %block length is equal to 64800
    block = llr(i * sstep + 1:i * sstep + sstep);
    dec_b_l(i * sstep / 2 + 1:i * sstep / 2 + sstep / 2) = step(decoderLDPC, block.');
end
toc

[Pbit_enc, errors] = SER(dec_b_l, b_l(1:length(dec_b_l)))
