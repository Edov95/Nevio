...
\subsection*{Charactarization of a single OFDM channel}
The system takes a sequence of input symbols $s_k$ at sampling time $T_{OFDM}=T_{block}/(M + N_{px})$ and applies an upsampling of factor 4, obtaining $a_k'$ at $T_c = T_{OFDM}/4$. This new sequence is then filtered by $g_{\sqrt{rcos}}$ with the as described by the following equation:
...
After the convolution with the transmission filter, we send the data through the channel with the inpulse response given by (\ref{eq:q_c}) and the white noise is added. From the following relations we can derive $\sigma_w^2$, the variance of the complex valued Gaussian noise:
\begin{equation}
...
At the reciever we have the matched filter of the transmission filter which is another $g_{\sqrt{rcos}}$ with the impulse response given by (\ref{eq:g_rcos}).
The output of the matched filter is then sampled at $T_{OFDM}$ starting from an initial offset called \emph{timing phase} $t_0$. We can just choose the value $\bar{t}_0$, multiple of $T/4$ and we have choosen $\bar{t}_0=90$ (91 with Matlab indexing) "because this is the value with the best performace"
