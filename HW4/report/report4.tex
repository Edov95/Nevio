...
At the receiver we have a matched filter $g_{M}$, obtained from $q_c$ as $g_M=q_c^*(t_0-t)$. For simplicity in the last formula we have denoted the filters as is they were defined on continuous time while in the actual simulation they are at $T/4$. 
...
\begin{equation}
l'_{2k}[i] =- \frac{2 \Re\{y_k[i]\}}{\sigma_{w}^2} \quad \quad
l'_{2k+1}[i]= - \frac{2 \Im\{y_k[i]\}}{\sigma_{w}^2}
\end{equation}

where we have the noise variance $\sigma_w$ which is the noise variance scaled by the maximum value of the psi impulse response. The resulting series of values is deinterleaved (see the first section for details on how the interleaver is built) and passed to the LDPC decoder that works with \emph{soft information}.
...
\begin{equation}
\Gamma = \frac{M_{s_c}}{N_0\frac{1}{T}} = \frac{\sigma_s^2 E_{g_{\sqrt{rcos}} * q_c}}{\sigma_w^2} \longrightarrow \sigma_w^2 = \frac{\sigma_a^2 E_{g_{\sqrt{rcos}} *q_c}}{\mathcal{M}\Gamma} = 2\sigma_I^2
\end{equation}
...
\begin{figure}[H]
	\begin{center}   
		\includegraphics[width=10cm]{figs/q_R.png} 
