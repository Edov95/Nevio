...
for Ncurrent=2:N
    ...
    h_est=h_est(1:Ncurrent);
    figure, stem(0:Ncurrent-1,h_est), hold on,
    stem(0:Ncurrent-1,h,'r*'), title('h_{analytic} vs h_{estimate-CORR}'), xlabel('n'), ylim([-0.5 1.2]), xlim([-2 20])
...
sw_collectCORR(Ncurrent)=sw_hatCORR_dB;
    ...
    h_ls=h_ls(1:Ncurrent);
    figure, stem(0:Ncurrent-1, h_ls),title('h_{ls}'), hold on,
    stem(0:Ncurrent-1,h,'r*'), title('h_{analytic} vs h_{estimate-LS}'), xlabel('n'), ylim([-0.5 1.2]), xlim([-2 20])
...
sw_collectCORR=sw_collectCORR(Nstart:N);
sw_collectLS=sw_collectLS(Nstart:N);
